...
namespace RenderBlog
{
    public static partial class Program
    {
        ...
        static readonly CultureInfo DefaultCulture = new("en-US");
        ...
        public static void Main(string[] args)
        {
            ...
            var filesParsing = frontMatterFiles.AsParallel()
                .Select(f =>
                {
                    ...
                    var localPath = isPost
                        ? f.path[(sitePath.Length + 1 + PostsFolder.Length + 1)..]
                        : f.path[(sitePath.Length + 1)..];
                    if (isMarkdown)
                    {
                    ...
                    frontMatter[IdKey] = isPost
                        ...
                        : null;
                    if (frontMatter.TryGetValue(TagsKey, out var tagsValue))
                    {
                        frontMatter[TagsKey] = ((List<object>)tagsValue).Cast<string>().ToList();
                    }
                    if (frontMatter.TryGetValue(TitleKey, out var titleValue))
                    {
                        frontMatter[TitleKey] = BetterTypography((string)titleValue);
                    }
                    ...
                    url = url.Equals(IndexFile, StringComparison.Ordinal) || url.EndsWith($"{UrlSeparator}{IndexFile}", StringComparison.Ordinal)
                        ? url[..Math.Max(url.Length - (IndexFile.Length + 1), 0)]
                        : url;
            ...
            foreach (var post in postsWithYears)
            {
                ...
                if (!yearsWithPosts.TryGetValue(year, out var value))
                {
                    value = [];
                    yearsWithPosts.Add(year, value);
                }
            ...
            filesParsing.AsParallel().ForAll(item =>
            {
                ...
                while (true)
                {
                    ...
                    var (path, frontMatter, content) = layouts[layout];
                    variables["content"] = pageContent;
                    pageContent = Render(content, variables, sitePath);

                    currentFrontMatter = ParseFrontMatter(frontMatter);

                    if (path.Equals(baseLayoutPath, StringComparison.Ordinal))
                        break;
                }
            ...
            foreach (var (path, frontMatter, content) in files.Where(x => x.frontMatter == null))
            {
                var localPath = path[(sitePath.Length + 1)..];
                var fileSystemPath = Path.Combine(outputFolder, localPath);
                EnsureDirectoryExists(fileSystemPath);
                File.Copy(path, fileSystemPath, true);
            }
        ...
        {
            var m = GetPostIdRegex.Match(filename);
            if (!m.Success)
                return null;
            return int.Parse(m.Groups[1].Value);
        }
        [GeneratedRegex(@"^(\d+)-.+")]
        static partial Regex GetPostIdRegex { get; }
        ...
        {
            var words = TimeToReadRegex.Count(text);
        ...
        [GeneratedRegex(@"\w+")]
        static partial Regex TimeToReadRegex { get; }
        ...
        static string BetterTypography(string s)
        {
            ...
            s = s.Replace(" - ", " – ");
            var toggle = true;
            s = DoubleQuotesRegex.Replace(s, match =>
            {
                if (toggle)
                {
                    toggle = !toggle;
        ...
        public static partial class MarkdownRenderer
        {
                ...
                foreach (var item in document.Descendants())
                {
                    switch (item)
                    {
                        ...
                        case HtmlInline inline:
                            {
                                var tag = inline.Tag;
                                tag = GenericsRegex.Replace(tag, "&lt;$1&gt;");
            ...
            [GeneratedRegex(@"<(T[a-zA-Z0-9]*)>")]
            static partial Regex GenericsRegex { get; }
